...
const Keyboard = dynamicImport(() => import('react-simple-keyboard'), { ssr: false });
...
const QuestionCreator = () => {
  ...
  const topicsByChapter: Record<string, string[]> = {
    ...
    'Chapter 1: Computer Basics': ['Topic 1: Computer Parts', 'Topic 2: Operating Systems'],
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setToast({ type: 'error', message: `Failed to submit question: ${(error as Error).message}` });
    }
  };
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      const metadataStr = String(metadataRow).replace('#', '').trim();
      ...
      const dataRows = rawData.slice(3).filter((row: any) => row && row.some((cell: any) => cell !== '' && cell !== null && cell !== undefined));
      ...
      setToast({
        type: 'info',
        message: `Processed ${validatedData.length} rows for ${fileSubject} - ${fileBook}, ${validatedData.filter((d: any) => d.errors.length === 0).length} valid`,
      });
    };
  ...
  const uploadBulk = async () => {
    const validQuestions = csvData.filter((d: any) => d.errors.length === 0);
    ...
    for (const { row } of validQuestions) {
      ...
      try {
        ...
        setToast({ type: 'error', message: `Failed to upload question: ${(error as Error).message}` });
        return;
      }
    }
  ...
  const urduPlaceholders = {
    ...
    blanks: (blankId: string) => `جوابات ${blankId} کے لیے (مثال: جواب1|جواب2)`,
  };
                ...
                <div className="space-y-4">
                  ...
                  <div className="border-t pt-4 mt-4">
                    ...
                    <div className="flex flex-col sm:flex-row gap-2 sm:gap-4">
                      <button
                        onClick={uploadBulk}
                        className={`min-h-[44px] px-4 py-2 rounded-lg text-sm font-medium ${isUploading || csvData.length === 0 || csvData.every((d: any) => d.errors.length > 0) ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-blue-600 text-white hover:bg-blue-700'}`}
                        disabled={isUploading || csvData.length === 0 || csvData.every((d: any) => d.errors.length > 0)}
                      >
